export function recoveryAdvice(error:string){
 if(/task allowance/i.test(error))return {kind:"billing",retry:false,message:"Review and increase the task allowance before retrying. Changing the image instructions will not fix this limit."};

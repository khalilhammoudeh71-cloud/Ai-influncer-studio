...
test('task allowance failures point to the allowance control rather than paid prompt repair',()=>{const advice=recoveryAdvice('Task allowance reached or not configured. Increase the limit before retrying. No new provider call was started.');assert.equal(advice.kind,'billing');assert.match(advice.message,/task allowance/i);});

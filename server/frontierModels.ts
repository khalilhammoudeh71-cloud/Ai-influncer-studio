...
export async function runFrontierChat(choice:string,key:string,messages:any[],system:string,request:typeof fetch=fetch){
 ...
 const response=await request(google?`https://generativelanguage.googleapis.com/v1beta/models/${selected.model}:generateContent`:'https://api.x.ai/v1/chat/completions',{
  ...
  body:JSON.stringify(google?{systemInstruction:{parts:[{text:system}]},contents:conversation.map(m=>({role:m.role==='assistant'?'model':'user',parts:[{text:m.content}]}))}:{model:selected.model,messages:[{role:'system',content:system},...conversation]}),
 });

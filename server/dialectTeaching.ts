...
export function createDialectTeachingRouter(dependencies:Partial<{read:typeof readVoiceState;write:typeof writeVoiceState;claim:typeof claimVoiceState;replace:typeof replaceVoiceState;analyze:(audio:string,dialect:string)=>Promise<any>;list:(owner:string)=>Promise<any[]>}>={}){
 ...
 router.post('/:dialect/recordings',async(req:AuthenticatedRequest,res)=>{
  ...
  const operationKey='dialect-recording:'+body.operationId,hash=createHash('sha256').update(id+audio).digest('hex');
  try{
   const previous=await read(owner,operationKey);
   if(previous){if(previous.hash!==hash)return res.status(409).json({error:'This request already refers to another recording.'});return res.json({id:body.operationId,status:previous.status,candidates:previous.candidates||[],error:previous.error});}
   ...
   const duplicate=stored.find((r:any)=>r.hash===hash&&r.status!=='deleted');if(duplicate)return res.json({id:duplicate.id,status:duplicate.status,candidates:duplicate.candidates||[],error:duplicate.error});
   const operation={hash,dialect:id,status:'analyzing',name:String(body.name||'Dialect reference').slice(0,100),audio,createdAt:new Date().toISOString()};
   ...
   try{
    const parsed=dependencies.analyze?await dependencies.analyze(audio,id):(await analyzeDialectAudio(audio,id,apiKey!)).data;const candidates=(Array.isArray(parsed.candidates)?parsed.candidates:[]).slice(0,12).map((r:any)=>dialectRule({...r,id:randomUUID(),recordingId:body.operationId})).filter(Boolean);
    await write(owner,operationKey,{...operation,status:'ready',candidates});res.json({id:body.operationId,status:'ready',candidates});

...
export function isConversationalMediaCreationRemark(value: unknown): boolean {
  ...
  const explicitCorrection = /(?:^|[،,.!?؟]\s*)(?:please\s+)?(?:change|edit|redo|regenerate|remake|make it|غيري|غيّري|عدلي|عدّلي|بدلي|بدّلي|اعملي|اعمل|ولدي)\s/iu.test(prompt);
  const identityMismatch = /(?:هذا|هاد|هدا|هاي|هذي|الشخص|الواحد|الزلمة|الرجل).{0,40}(?:مش|مو|ليس)\s*(?:أنا|انا)|(?:الشخص|الصورة|الصوره|الواحد).{0,100}(?:ما بيشبهني|مش شبهي|مو شبهي)|(?:person|man|woman|that|this|image|photo|picture).{0,60}(?:is not me|isn't me|does not look like me|doesn't look like me|not my face|not me)/iu.test(prompt);
  if (identityMismatch && !explicitCorrection) return true;

...
export function requiresTranscriptConfirmation(data:any,draft='') {
  ...
  if(typeof data?.text!=='string'||!data.text.trim()||!probs.length)return true;
  if(probs.reduce((n:number,p:any)=>n+p.logprob,0)/probs.length<-.65||probs.some((p:any)=>p.logprob<-2.5))return true;

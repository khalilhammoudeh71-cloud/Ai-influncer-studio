...
Verification: 18 focused tests plus 8 conversation-state tests including an embedded PostgreSQL integration test covering duplicate submission, cross-account isolation, pause/resume, dependent edit source selection, failure stopping, explicit retry, and successful completion. Browser fixture checks covered approval, pause, resume, and reload restoration. Type checking, frontend build and API bundle passed.

Limits: no claim of provider-side exactly-once execution; existing stale media-job recovery can resubmit a provider request. The paused plan does not cancel a provider request already sent. Worker advancement can take approximately a minute between steps. Plans containing unsupported operations still need the browser. Live deployment verification: the custom-domain worker accepted the saved two-step plan, recovered its queued child after the timestamp fix while the app tab was closed, and correctly stopped on insufficient app credits (3 required, 0 available). Reopening showed the error and explicit retry control. The dependent edit was not started. Successful live provider completion remains unverified because of the credit balance.

Integration test uses @electric-sql/pglite 0.5.8 installed temporarily for this test, with PGLITE_MODULE pointing to its ESM entry. No production credentials or paid generation were used by automated tests. The live planner used the configured account; the image request was rejected before provider generation. Browser settings were restored to xAI Grok and the original mode.

The live test also identified and fixed PostgreSQL microsecond timestamps preventing the existing worker claim, plus custom edit labels being misclassified as browser-only. Regression tests cover both.

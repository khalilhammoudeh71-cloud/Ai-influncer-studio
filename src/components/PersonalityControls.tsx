...
export default function PersonalityControls({ persona, onChange }: Props) {
 ...
 return <div className="space-y-5">
  ...
  <label className="flex items-start gap-3 text-sm text-white"><input type="checkbox" className="mt-1 accent-[#E7C477]" checked={settings.voiceEnabled} onChange={e=>onChange({...settings,voiceEnabled:e.target.checked})}/><span>Match spoken delivery to personality<span className="block text-xs text-slate-400 mt-1">Uses the selected traits to adjust supported pacing and expression controls. Keeps the selected speaker. Turn off to use your existing voice settings. Save changes and start a new call to apply them.</span>{!traits.length && <span className="block text-xs text-slate-400 mt-1">Choose at least one personality trait above to shape spoken delivery.</span>}</span></label>

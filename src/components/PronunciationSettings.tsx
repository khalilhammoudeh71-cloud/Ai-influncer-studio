...
export function PronunciationSettings({personaId,onPreview,revision=0,futureCallOnly=false}:{personaId:string;onPreview?:(text:string)=>void;revision?:number;futureCallOnly?:boolean}) {
  ...
  return <details className="rounded-xl border border-white/10 p-3 text-sm"><summary className="cursor-pointer text-[#E7C477]">Remembered pronunciations {loading?'':`(${rules.length})`}</summary>
    ...
    <div className="mt-3 grid grid-cols-2 gap-2"><input aria-label="Word or phrase" dir="auto" placeholder="Word or phrase" value={word} onChange={e=>setWord(e.target.value)} maxLength={160} className="min-h-11 min-w-0 rounded-lg bg-black/30 p-2"/><input aria-label="Say it as" dir="auto" placeholder="Say it as…" value={spokenAs} onChange={e=>setSpokenAs(e.target.value)} maxLength={200} className="min-h-11 min-w-0 rounded-lg bg-black/30 p-2"/></div>
    <button type="button" className="mt-2 min-h-11 rounded-lg bg-[#E7C477] px-3 py-2 text-black disabled:opacity-40" disabled={busy||loading||!word.trim()||!spokenAs.trim()} onClick={()=>void save()}>{busy?'Saving…':'Save pronunciation'}</button>{error&&<p role="alert" className="mt-2 text-amber-300">{error}</p>}
  </details>;
}

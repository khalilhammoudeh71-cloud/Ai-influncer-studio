...
export default function CarouselSequenceStudio({persona,slides,format,onUpdate,onBusy,photos,selected,onSelect}:{selected:number;onSelect:(i:number)=>void;photos:string[];persona:Persona;slides:CarouselSlide[];format:'instagram'|'tiktok';onUpdate:(index:number,patch:Partial<CarouselSlide>)=>void;onBusy:(message:string)=>void}){
 ...
 return <section aria-label="Photo sequence generator" className="space-y-5">
  <div><h3 className="font-semibold text-lg">{selected===0?'Generate your cover':`Generate slide ${selected+1}`}</h3><p className="text-xs text-slate-400 mt-1">Create a new shot with your persona’s identity and your chosen image model. Generation uses your existing API credits.</p></div>
  <div className="space-y-3"><label className="text-xs">Image model<select value={modelId} onChange={e=>setModelId(e.target.value)} className="luxury-input w-full p-3 mt-2"><option value="" disabled>Choose an image model</option>{models.map(m=><option value={m.id} key={m.id}>{m.name}</option>)}</select><span className="block text-slate-400 mt-2">Available models that support a reference image. Results and identity consistency vary by model.</span></label></div>
  {selected===0&&<div className="space-y-4"><CarouselCoverBuilder format={format} reference={coverReference} photos={photos} choices={coverChoices} onReference={coverReference=>onUpdate(0,{coverReference})} onChoices={coverChoices=>onUpdate(0,{coverChoices})} onBusy={onBusy} onError={setError}/><label className="block text-xs">Describe the photo<textarea value={coverPrompt} onChange={e=>onUpdate(0,{imagePrompt:e.target.value})} maxLength={1800} rows={3} placeholder="A candid café portrait in warm morning light, wearing a cream jacket." className="luxury-input w-full mt-2 p-3"/></label><button type="button" disabled={!modelId||!hasCoverIdea} onClick={generateCover} className="btn-gold-primary w-full px-3 py-3 text-sm disabled:opacity-40">Generate cover</button><p className="text-xs text-slate-400">Uses your selected model, generation reference, style choices, and prompt. Replaces the current cover photo.</p></div>}
  ...
  <div className="space-y-4">{targets.filter(i=>i===selected).map(i=><div key={i} className="rounded-xl border border-white/15 p-3 space-y-3"><div className="flex items-center gap-3">{slides[i].image&&<img src={slides[i].image} alt={`Current slide ${i+1}`} className="h-20 w-16 object-cover rounded-lg"/>}<h4 className="font-semibold text-sm">Slide {i+1}</h4></div>
   ...
   <button type="button" disabled={!cover||!modelId||!slides[i].imagePrompt?.trim()} onClick={()=>generate([i])} className="btn-gold-primary w-full px-3 py-3 text-sm disabled:opacity-40">Generate slide {i+1}</button>
  </div>)}</div>
  <details className="border-t border-white/15 pt-4"><summary className="cursor-pointer text-sm font-medium">Plan & generate remaining slides</summary><div className="space-y-4 mt-4"><label className="text-xs">Sequence direction<select value={style} onChange={e=>setStyle(e.target.value)} className="luxury-input w-full p-3 mt-2">{SEQUENCE_STYLES.map(([id,name])=><option value={id} key={id}>{name}</option>)}</select></label>  <label className="block text-xs">What should happen next? (optional)<textarea rows={2} maxLength={2000} value={instructions} onChange={e=>setInstructions(e.target.value)} placeholder="Keep the outfit and café. Suggest a close-up, a wider view, then walking away." className="luxury-input w-full p-3 mt-2"/></label>
  ...
  <button type="button" disabled={!ready} onClick={()=>generate(targets)} className="btn-gold-secondary px-4 py-3 text-sm disabled:opacity-40">Generate all {targets.length} following slides</button><p className="text-xs text-slate-400">Generation replaces each target slide’s photo. Successful photos remain if another slide fails. Slides generate one after another using the selected model; you can retry individual slides.</p>
</div></details>

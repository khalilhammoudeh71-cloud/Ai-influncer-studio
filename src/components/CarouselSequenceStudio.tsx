...
export default function CarouselSequenceStudio({persona,slides,format,onUpdate,onBusy,photos}:{photos:string[];persona:Persona;slides:CarouselSlide[];format:'instagram'|'tiktok';onUpdate:(index:number,patch:Partial<CarouselSlide>)=>void;onBusy:(message:string)=>void}){
 ...
 const suggest=async()=>{
  ...
  try{
   ...
   if(alive.current){setSuggestions(valid);valid.forEach(s=>onUpdate(s.slide,{imagePrompt:s.options[0].prompt}));setStatus('Suggestions are ready. Choose an option or edit the prompt for each slide.');}
 ...
 return <section aria-label="Photo sequence generator" className="space-y-4 rounded-xl border border-[#E7C477]/40 p-4 sm:p-5">
  <div><h3 className="font-semibold text-lg">Generate your cover and photo sequence</h3><p className="text-xs text-slate-400 mt-1">Use the cover to plan related photos, then generate them with your existing image models. Your cover stays unchanged. Each generated image uses API credits.</p></div>
  <details className="rounded-lg border border-white/15 p-3"><summary className="cursor-pointer text-sm">Choose a different cover photo</summary><div className="mt-3"><CarouselPhotoPicker label="Cover photo" photos={photos} value={cover} onChange={image=>onUpdate(0,{image})}/></div></details>
  {cover?<img src={cover} alt="Sequence cover reference" className="w-28 h-32 object-cover rounded-lg"/>:<p className="text-sm text-amber-200">Choose a photo for the cover slide first.</p>}
  ...
  <div className="space-y-4">{targets.map(i=><div key={i} className="rounded-xl border border-white/15 p-3 space-y-3"><div className="flex items-center gap-3">{slides[i].image&&<img src={slides[i].image} alt={`Current slide ${i+1}`} className="h-20 w-16 object-cover rounded-lg"/>}<h4 className="font-semibold text-sm">Slide {i+1}</h4></div>

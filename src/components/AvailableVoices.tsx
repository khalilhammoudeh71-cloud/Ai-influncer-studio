...
import { STOCK_VOICES, type StockVoice } from '../../shared/stockVoices';
export default function AvailableVoices({voices,selectedId,selectedEngine,onSelect,disabled}:{voices:StockVoice[];selectedId:string;selectedEngine:string;onSelect:(voice:StockVoice)=>void;disabled:boolean}) {
 ...
 const visible=all.filter(v=>Object.entries(filters).every(([k,value])=>!value||v[k as keyof typeof filters]===value)&&`${v.name} ${v.provider} ${v.tone}`.toLowerCase().includes(search.toLowerCase()));
 ...
 <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">{(Object.keys(filters) as Array<keyof typeof filters>).map(key=><label key={key} className="text-xs capitalize text-slate-400">{key}<select aria-label={`Filter voices by ${key}`} className="luxury-input mt-1 w-full p-2 text-xs" value={filters[key]} onChange={e=>setFilters({...filters,[key]:e.target.value})}><option value="">All</option>{[...new Set(all.map(v=>v[key]))].sort().map(value=><option key={value}>{value}</option>)}</select></label>)}</div>

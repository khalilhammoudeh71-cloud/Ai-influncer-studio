...
export default function CarouselDesignControls({slide,photos,onChange,onPanorama,photosOnly=false,format}:{format:'instagram'|'tiktok';photosOnly?:boolean;slide:CarouselSlide;photos:string[];onChange:(s:Partial<CarouselSlide>)=>void;onPanorama:()=>void}){
 ...
 <label className="block text-xs">Layout<select className="luxury-input w-full mt-2 p-3" value={photosOnly&&(!slide.layout||['editorial','minimal'].includes(slide.layout))?'full-photo':slide.layout||'editorial'} onChange={e=>onChange({layout:e.target.value as CarouselLayout})}>{LAYOUTS.filter(l=>!photosOnly||!['minimal','editorial'].includes(l.id)).map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label>
 {(slide.layout==='split'||slide.layout==='scrapbook')&&<CarouselPhotoPicker format={format} label="Second photo" photos={photos} value={slide.secondImage} onChange={secondImage=>onChange({secondImage})} emptyLabel="Repeat first photo"/>}
 {(['cropX','cropY','zoom','textScale'] as const).filter(k=>!photosOnly||k!=='textScale').map(k=><label key={k} className="block text-xs">{{cropX:'Photo horizontal position',cropY:'Photo vertical position',zoom:'Photo zoom',textScale:'Text size'}[k]}<input className="block w-full mt-2 accent-[#E7C477]" type="range" min={k==='zoom'?1:k==='textScale'?.7:0} max={k==='zoom'?3:k==='textScale'?1.3:100} step={k==='zoom'||k==='textScale'?.05:1} value={slide[k]??(k==='zoom'||k==='textScale'?1:50)} onChange={e=>onChange({[k]:+e.target.value})}/></label>)}
 {!photosOnly&&<div className="grid grid-cols-2 gap-3"><label className="text-xs">Text alignment<select className="luxury-input w-full mt-2 p-2" value={slide.align||'left'} onChange={e=>onChange({align:e.target.value as 'left'|'center'})}><option value="left">Left</option><option value="center">Centered</option></select></label><label className="text-xs">Font<select className="luxury-input w-full mt-2 p-2" value={slide.font||'serif'} onChange={e=>onChange({font:e.target.value as 'serif'|'sans'})}><option value="serif">Editorial serif</option><option value="sans">Clean sans serif</option></select></label></div>}
 {!photosOnly&&(slide.layout==='full-photo'||slide.layout==='panorama')&&<label className="block text-xs">Text position<select className="luxury-input w-full mt-2 p-2" value={slide.textPosition||'middle'} onChange={e=>onChange({textPosition:e.target.value as 'top'|'middle'|'bottom'})}><option value="top">Top</option><option value="middle">Middle</option><option value="bottom">Bottom</option></select></label>}
